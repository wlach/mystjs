...
describe('Utils', () => {
    it('html formatting is simple', () => {
        const f = (0, utils_1.formatTag)('figure', { id: 'one', class: 'numbered' }, false);
        expect(f).toBe('<figure id="one" class="numbered">');
    });
    it('strips dangerous tags', () => {
        const f = (0, utils_1.formatTag)('figure', { id: 'one', class: '<script>' }, false);
        expect(f).toBe('<figure id="one" class="&lt;script&gt;">');
    });
});
describe('toHTML', () => {
    it('Converts a tag schema to a string', () => {
        const [a, b] = (0, utils_1.toHTML)([
            'figure',
            { hi: '1' },
            ['img', { src: '2' }],
