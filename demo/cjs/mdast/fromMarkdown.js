    ...
    Object.entries(specHandlers).forEach(([type, spec]) => {
        ...
        if (noCloseToken(spec, type)) {
            handlers[type] = (state, tok, tokens, i) => {
                if (spec.isText) {
                    state.addText((0, utils_1.withoutTrailingNewline)(tok.content), spec.type, attrs(spec, tok, tokens, i));
                    return;
                }
                state.openNode(nodeType, attrs(spec, tok, tokens, i), spec.isLeaf);
                state.addText((0, utils_1.withoutTrailingNewline)(tok.content));
                state.closeNode();
            };
        }
        else {
